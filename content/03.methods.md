...
Journalistic articles were defined as all articles from 2005 to 2020 that were designated as "News", "News Feature", "Career Feature", "Technology Feature", and "Toolbox".
Using the spider “target_year_crawl.py”, we scraped the title and main text from all journalistic articles.
...
Using an additional spider defined in “doi_crawl.py”, we scaped all citations within journalistic articles.
...
After the articles were scraped and processed, the text was processed using the coreNLP pipeline [@doi:10.3115/v1/P14-5010] (version 4.2.0).
...
We generated a comparative background set for supplemental analysis with the _Springer Nature_ API by obtaining author information for research articles cited in journalistic articles.
We selected a random set of articles to generate the _Springer Nature_ background set.
These articles were the first 200 English language "Journal" articles returned by the _Springer Nature_ API for each month, resulting in 2400 articles per year for 2005 through 2020.
To get the author information for the cited articles, we queried the _Springer Nature_ API using the scraped DOI.
...
This processing was not required for quote prediction because names written in journalistic articles did not appear to be reversed or abbreviated.
...
For names identified in the _Nature_ journalistic articles, we use the same process as described for the gender prediction; we again try to identify the full name.
...
In addition, we also take all authors cited in a journalistic _Nature_ article.
...
We calculated a countries' citation rate as the number of citations for a country divided by either the number of _Nature_ research articles (@eq:mention-bg) or the total number of papers cited by journalistic articles for that year (@eq:mention-news).
...
Once we identified set C/M countries, we analyzed the word frequencies in all journalistic articles where the set C or M country was mentioned but not cited.
...
The bootstrap estimates were taken by subsampling the journalistic articles with replacement, each time recalculating the country-normalized token frequencies within each country set (C and M).
