...
If we are still unable to find a full name, or if coreNLP cannot identify a speaker at all, we also look if coreNLP identified a gendered pronoun to the quote.
If so, we assign the gender of the speaker to the gender of the pronoun.
...
Only articles with more than one author were considered.
Similarly for the name formatting for quotes, we need to extract the first name of the authors.
...
This processing was not done in the quotes prediction because we assume the names written in an article are not reversed or abbreviated.
...
Additionally, if a full name is less than three characters, it is not considered.
