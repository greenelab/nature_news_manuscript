...
Panel A, left, depicts an example journalistic article and the type of data extracted from the text.
...
Panel A, right, charts the analyses done on the extracted names and locations from journalistic articles, research articles, and letters published by _Nature_.
...
We have analyzed the text and citations of 22,001 journalism-related articles hosted on "www.nature.com" that span 15 years from 2005 to 2020.
...
We also include one analysis of the scientist-written journalistic articles, "Career Column" and "News and Views", as an additional set of 5,921 articles.
...
To more broadly represent the overall science authorship, we also separately analyzed 36,000 randomly selected _Springer Nature_-published articles from English language journals over the same time.
...
We observed substantial variability in the number ofjournalistic _Nature_ articles by type between 2005 and 2020 (Figure {@fig:fig1}b).
The changing classification of article types may explain temporal changes in journalistic articles.
Over time, the frequency of "News" articles decreased; however, more specific journalism-related article types increased, including the introduction of the new categories "Career Feature", "Toolbox", and "Career Column".


### Quoted Speakers and Primary Research Authors in _Nature_ are More Often Male

![
**Predicted male speakers are overrepresented in journalistic quotes, but this depends on the article type.**
Panel A, left, depicts an example of the names extracted from quoted speakers in journalistic articles and authors in research articles.
...
Performance of binary prediction was evaluated on a benchmark data set of thirty randomly selected journalistic articles, ten from each of the following years: 2005, 2010, 2015 (Figure {@fig:suppfig1}a).
...
We first examined the number of quotes identified within each type of journalistic-related article (Figure {@fig:fig2}b), totaling 119,998 quotes with 109,723 of them containing a gender prediction for the speaker.
Quote frequencies vary by article type.
We compared the number of quotes from predicted male persons to the number of predicted male first and last authors published in _Nature_.
...
The predicted gender gap between first and last authors was larger in our selection of _Springer Nature_ articles; however, both first and last authors were predicted to be closer to parity than for _Nature_ authors.
Overall, predicted male persons were more frequently quoted than predicted female persons in _Nature_ journalistic articles and published first and last primary research authors in _Nature_ and _Springer Nature_ over the same time period. 


We analyzed a total of 10,454 first authors and 10,488 last authors with a gender prediction.
As denoted by the red line, we find that both _Nature_ authorship and quotes are far from gender parity.
...
Since _Nature_ authorship may not represent scientific publishing as a whole, we also compared against a random selection of authors from English language journals published by _Springer Nature_ (Figure {@fig:suppfig2}a).
We observed a larger gender gap between first and last authors in our selection of _Springer Nature_ articles; however, both first and last authors are much closer to parity than _Nature_ authors.

The gender proportions of authorship were relatively stable over time for both _Nature_ and _Springer Nature_ articles.
In contrast, we found that the rate of quotes predicted to be from male persons significantly decreased over time.
In 2005, the fraction of quotes predicted to be from male persons was 86.87% (5,552/6,391) whereas in 2020 it was 68.5% (3,494/5,098).
Indeed, the fraction of quotes from predicted male persons was initially higher than the fraction of predicted male last authors, then slowly decreased until it was below the predicted male first and last authorship rates in 2020.
...
We found that quotes trend slightly towards last authors from 2005 to 2020, but because the fraction of predicted male last authors remained stable over time both for _Nature_ and the selection of _Springer Nature_ articles, this likely does not explain the downward trend.
...
### Predicted Celtic English Name Origins are over enriched in cited and quoted persons, while predicted East Asian name origins are under enriched
...
Panel A, left, depicts an example of the names extracted from quoted speakers and citations found within journalistic articles and authors in research articles.
Panel A, right, highlights the data types and processes used to analyze the predicted origin of extracted names. Panels B and C depict a comparison between the predicted name origins of last authors in _Nature_ and cited articles in journalistic articles.
Panel B and C differ in the journalistic article types.
...
In addition, we also identified the last authors of all articles cited by a journalistic article. All processed names were then input into Wiki2019-LSTM and assigned one of ten possible name origins ([Methods]).
Figure {@fig:fig3}a shows an overview of the process and example input data for this analysis: 1) quotes and quoted speakers (blue box), 2) names of cited last authors in journalistic articles (green)  3) last authors' names of Research Articles and Letters published by _Nature_ (dark orange box).
We divided our analysis into three parts: firstly, quantifying the proportions of predicted name origins of last authors cited in journalistic _Nature_ articles.
...
Additionally, in our supplemental analyses, we compared against the last authorship in a random selection of _Springer Nature_ articles.
We found that the number of quotes and unique names mentioned dramatically outnumbered the number of cited authors in journalistic _Nature_ articles, as well as  last authors within _Nature_ primary research articles (Figure {@fig:suppfig3}a).
...
Minimum and median per data type over all years: _Nature_ articles, (565, 679); _Springer Nature_ articles, (1298, 1684); quotes, (4577, 6194); mentions, (3634, 5002); citations in journalist-written article, (139, 267) citations in a scientist-written article, (503, 660).

In comparing the citation rate of last author name origins in journalistic articles, we decided to additionally analyze scientist-written articles.
...
We found a slight over-enrichment for predicted Celtic/English name origins and a small under-enrichment for predicted East Asian name origins in scientist-written and journalist-written articles when compared to the composition of last authors in _Nature_ (Figure {@fig:fig3}b, c).
...
We also observed the predicted Celtic/English over-enrichment and East Asian under-representation when considering our subset of _Springer Nature_ articles (Figure {@fig:suppfig4}b) for both journalist- and scientist-written articles.
...
Additionally, we see a significant difference in predicted Arabic/Turkish/Persian name origins frequencies between cited authors and _Springer Nature_ authors, however the difference is lower than observed for Celtic/English and East Asian predicted name origins.
...
Additionally, we also found a much stronger depletion of quotes from people with predicted East Asian name origins (Figure {@fig:suppfig3}b), with never more than 7.9% of quotes even though they constitute between 5.7-24.8% of last authors cited in either journalist- or scientist-written articles (Figure {@fig:fig3}b,c).
...
![
**Type of country representation in journalistic articles differ across countries**
Panel A, left, depicts an example of the country mentions extracted from the journalistic article text and citations found within journalistic articles and author affiliations in research articles.
...
After finding name origin differences between cited and quoted persons in comparison to last authorship rates, we wanted to determine if journalistic articles 1) represent countries at different rates, or 2) vary in the language used to describe scientific content related to each country.
To perform this analysis, we used three sources of information: 1) country-related entities mentioned in the journalistic article text (light orange), 2) country affiliations of cited authors in journalistic articles (green), 3) country affiliation of authors in _Nature_ and _Springer Nature_ (dark orange).
...
Affiliation query results from the _Springer Nature_ API return all country affiliations for a specific manuscript and are not linked to one particular author.
 
After post-processing, we analyzed a total of 1,989 articles with a citation accessible through the _Springer Nature_ API.
...
We quantified this by counting all the journalist-written articles in which a country, region within a country, or organization affiliated with a country was mentioned, which we term a country's "mention rate".
