...
Next, we identified if the journalist name origin had any effect on quote disparities.
...
However, this disparity was reduced when only considering quotes from people cited within the same news article, and reduced further when considering only citations with a US affiliation (Table {@tbl:table6} and {@tbl:table7}).
