...
We found that the under-enrichment of East Asian name origins was greatly reduced, more closely matching the analysis on citations alone (Figure {@fig:suppfig_quote_cite}a,b).
Next, we designed an experiment to test if the predicted journalist name origin had any effect on quote disparities.
...
To interrogate the source of this difference between journalists with different predicted name origins, we looked at a more specific subsection of quotes.
...
We found that differences between journalists with different predicted name origins was nearly eliminated when additionally considering quoted speakers with a US affiliated citation, as evidenced in the East Asian column of Table {@tbl:table7}.
While we are unable to identify the location of the journalists at the time of writing the articles, this suggests that source gathering of journalists may have a regional bias.
