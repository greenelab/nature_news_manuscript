...
From the coreNLP pipeline results, we wanted to identify two distinct types of data to analyze, country mentions and quotes with their associated speakers.
...
We then mapped the named entity to a country prediction using Open Street Map [@https://www.openstreetmap.org].
...
The author metadata of research-related articles were analyzed over the same period as the news-related articles and totaled 13,414 articles.
...
Panel A, right, highlighted the data types and processed used to analyze the gender of extracted names.
...
Figure @fig:fig2a shows an overview of the process and example input data for this analysis.
...
Since Nature authorship may not represent scientific publishing as a whole, we also compared against a random selection of authors from English language journals published by Springer (Supplemental Figure 2a).
...
We found that quotes are slightly biased towards the last authors, however, this rate does not change over time, and we do not believe this drives the downward trend.
We then analyzed the breakdown of gendered quotes by article type.
Interestingly, we found one article type, "career-feature" that has achieved gender parity in its quotes (Figure @fig:fig2 e and Supplemental Figure 2b).
...
Figure @fig:fig3a displays a schematic of the data processing and example names used in this analysis.
...
We find that the number of quotes dramatically outnumbers the number of authors and citations (Supplemental Figure 3A). 
...
We find a slight over-enrichment for Celtic/English names and a slight under enrichment for East Asian names.
Furthermore, we find no significant difference for European nor any other name origin (Figure @fig:fig3 c, Supplemental Figure 3B).
We also observed the Celtic/English over-enrichment and East Asian under-enrichment when considering our subset of Springer articles (Supplemental Figure 3C).
However, we also see a significant difference in European names, with a growing over-enrichment.
Additionally, we see a significant difference in Arabic/Turkish/Persian, Hispanic, Jewish, and South Asian name frequencies between cited authors and Springer authors, however, at a lower rate than observed for both Celtic/English and East Asian.
...
Panel E shows the top 15 words extracted from articles mentioning the countries depicted in Panel D, with the most significant proportional frequency between the two defined country classes.
...
Figure @fig:fig4a shows example input data and a schematic of the analysis.
...
Interestingly, we found a strong citation over-enrichment of many top-cited countries, but we found no evidence of under-enrichment by East Asian countries (Figure @fig:fig4 c, Supplemental Figure 4A).
...
To do this, we calculated the citation and mention rate of each country and identify countries that had a consistent skew towards either a higher or lower mention rate (Figure @fig:fig4 d and Supplemental Figure 4B).
This outlier description allowed us to identify two classes of countries: high relative citation to mention rate: Germany, Netherlands, Japan, Spain, Denmark, Israel, Sweden, Switzerland, Belgium, Austria, China, South Korea, and Australia; low relative citation to mention rate: Canada, Democratic Republic of the Congo, Chile, Indonesia, Russia, South Africa, Mexico, Brasil, Philippines, Colombia,  and India.
...
After proper identification of high-frequency words across the entire corpus, we identified the top 15 most discriminative terms of each country type [Methods].
...
The top 5 terms were rainforest, pesticide, dams, plantations, and deforestation (Figure @fig:fig4 e, Supplemental Figure 4C and D).
