...
We found that differences between journalists with different predicted name origins was nearly eliminated when restricting to quoted and cited speakers, with marginal improvement with an additional restriction of US affiliated citations, as evidenced in the predicted East Asian column of Table {@tbl:table7}.
The differences between Table {@tbl:table5} and Tables {@tbl:table6} and {@tbl:table7}, indicate that the predicted name origin of a journalist has some effect on sources gathered outside of directly cited works.
Since the additional location based constraint of having a US-affilitated citation induces only a marginal change, we are unable to identify if there is a significant regional bias.    
Furthermore, we do not know the location of the journalists at the time of writing the articles.
Nevertheless, our findings support the hypothesis that there exist name-origin specific biases of journalists when identifying sources beyond authors directly cited within an articles.
