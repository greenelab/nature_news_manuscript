...
We have analyzed the text and citations of 16,080 news-related articles hosted on "www.nature.com".
The articles span 15 years from 2005 to 2020, and five article types: Career Feature, News, News Feature, Technology Feature, and Toolbox.
In our analyses, we focus primarily on journalist-written articles, but do include one analysis of scientist-written articles in the columns, Career Column and News and Views, which adds an additional 5,921 articles.
...
This is because scientist-written articles have many citations and could help disentangle the bias effect between scientists and journalists.
...
Interestingly, we find that the under-enrichment for East Asian names in journalist-written articles is only observable from 2005 to 2009.
...
We also observed the Celtic/English over-enrichment and East Asian under-enrichment when considering our subset of _Springer_ articles (Figure {@fig:suppfig4}b) for both journalist- and scientist-written articles.
In contrast to _Nature_, in our _Springer_ articles, we see a difference in European names, with a growing over-enrichment.
...
Additionally, we also found a much stronger under-enrichment of quotes from East Asian speakers (Figure {@fig:suppfig3}b), with never more than 7.9% of quotes even though they constitute between 5.7-24.8% of cited authors.
When we again compare against last authorship in _Nature_, we find the same patterns observed in the previously described citation analysis with all name origins except for East Asian and Celtic/English closely matching the background rate (Figure {@fig:fig3}d).
...
Since quoted speakers may be biased towards English speakers, we expanded our analysis to also include sources that were possibly paraphrased and not directly quoted. 
To do this, we identified all unique names that were identified in an article.
