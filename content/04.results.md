...
We found this result consistent for articles written by either a predicted male or female journalists (Figure {@fig:suppfig_j_gender}a,b).
...
Furthermore, we also found that the Celtic/English over-enrichment was much stronger in _The Guardian_ in comparison to _Nature_.
...
We found that the disparity was greatly reduced, more closely matching the analysis on citations alone (Figure {@fig:suppfig_quote_cite}a,b).
Next, we identified if the journalist name origin had any effect on quotes disparities.
When considering all quotes, we found that journalists with a predicted East Asian name origin had a higher rate of East Asian quoted speakers (Table {@tbl:table5}).
However, this disparity was reduced when only considering quotes from people cited within the same news article, and reduced further when considering only considering citations with a US affiliation (Table {@tbl:table6} and {@tbl:table7}).
...
The top 5 terms were "rice", "amazon", "astronauts", "monsanto", and "forest" (Figure {@fig:fig4}e, Figure {@fig:suppfig4}c,d).
